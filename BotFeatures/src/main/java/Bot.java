				...
				if (update.callbackQuery() != null) {
					...
					if (update.message().text() != null) {
						...
						if (update.message().text().equals("/keyboard")) {
							...
							sendResponse = bot
									.execute(new SendMessage(update.message().chat().id(),"Mensgem original aguarde..."));
							new Thread();
							Thread.sleep(2000);
							baseResponse = bot
									.execute(new EditMessageText(update.message().chat().id(), sendResponse.message().messageId(), "Mensagem editada"));
